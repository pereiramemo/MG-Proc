...
### described in the project CLAUDE.md, mirroring bin/utils.py so the Python
...
count_phix_seqs <- function(p, n_sample = 10000, seed = 123) {
  set.seed(seed)
  # Reservoir-sample n_sample reads in one streaming pass (bounds memory).
...
# First two bytes of a file, for compression detection (mirrors bin/utils.py).
.file_magic <- function(path) {
  con <- file(path, "rb")
  on.exit(close(con))
...
# src when src is already plain. Needed before handing reads to tools that
# only understand gzip or nothing at all (e.g. fastp, ShortRead's qa()/
# FastqSampler() don't read bzip2). Returns the detected kind: "gzip",
# "bzip2", or "plain".
decompress_or_link <- function(src, dst) {
  sig <- .file_magic(src)
  is_gzip <- length(sig) == 2 && sig[1] == as.raw(0x1f) && sig[2] == as.raw(0x8b)
  is_bzip2 <- length(sig) == 2 && rawToChar(sig) == "BZ"

  if (is_gzip || is_bzip2) {
    con_in <- if (is_gzip) gzfile(src, "rb") else bzfile(src, "rb")
    on.exit(close(con_in))
    con_out <- file(dst, "wb")
    on.exit(close(con_out), add = TRUE)
    repeat {
      chunk <- readBin(con_in, what = "raw", n = 1e7)
      if (length(chunk) == 0) break
      writeBin(chunk, con_out)
    }
    return(if (is_gzip) "gzip" else "bzip2")
  }
